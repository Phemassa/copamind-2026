...
function buildScatterCanvas(rows, logo) {
  ...
  bg.addColorStop(0, "#0b1222"); bg.addColorStop(1, "#161a2a");
  ctx.fillStyle = bg; ctx.fillRect(0, 0, W, H);

  // Dados
  const maxPts   = Math.max(...rows.map((r) => r.net_score || r.points || 0), 1);
  const minPts   = Math.min(...rows.map((r) => r.net_score || r.points || 0), 0);
  const maxScore = Math.max(...rows.map((r) => r.scored || 0), 1);
  const accMin   = 0.55, accMax = 1.0;

  const toX = (pts) => PAD.left + ((pts - minPts) / Math.max(1, maxPts - minPts)) * PW;
  const toY = (acc) => PAD.top  + (1 - (acc - accMin) / (accMax - accMin)) * PH;
  const toR = (scored) => 5 + Math.min(14, scored * 0.9);

  // Grid
  ctx.strokeStyle = "#ffffff14"; ctx.lineWidth = 1;
  [0.6, 0.7, 0.8, 0.9, 1.0].forEach((acc) => {
    const y = toY(acc);
    ctx.beginPath(); ctx.moveTo(PAD.left, y); ctx.lineTo(PAD.left + PW, y); ctx.stroke();
    ctx.fillStyle = C.muted; ctx.font = `${10 * S / S}px system-ui`; ctx.textAlign = "right";
    ctx.fillText(`${Math.round(acc * 100)}%`, PAD.left - 8, y + 4);
  });
  const xTicks = 5;
  for (let i = 0; i <= xTicks; i++) {
    const pts = Math.round(minPts + (i / xTicks) * (maxPts - minPts));
    const x = toX(pts);
    ctx.beginPath(); ctx.moveTo(x, PAD.top); ctx.lineTo(x, PAD.top + PH); ctx.stroke();
    ctx.fillStyle = C.muted; ctx.textAlign = "center";
    ctx.fillText(String(pts), x, PAD.top + PH + 18);
  }
  ...
  // Título
  ctx.fillStyle = C.text; ctx.textAlign = "center"; ctx.font = `bold 22px system-ui`;
  ctx.fillText("Acurácia × Pontos Efetivos", W / 2, 32);
  ctx.fillStyle = C.muted; ctx.font = `12px system-ui`;
  ctx.fillText("Tamanho do círculo = jogos pontuados  •  passe o mouse no portal para detalhes", W / 2, 50);

  // Labels de eixo
  ctx.fillStyle = C.muted; ctx.font = `11px system-ui`;
  ctx.textAlign = "center";
  ctx.fillText("Pts Efetivos →", PAD.left + PW / 2, H - 12);
  ctx.save(); ctx.translate(16, PAD.top + PH / 2);
  ctx.rotate(-Math.PI / 2); ctx.fillText("Acurácia %", 0, 0); ctx.restore();

  // Pontos (bolhas)
  const FAMILY_COLORS = {
    gemma: "#4285f4", qwen: "#1aab8b", phi: "#0078d4", mistral: "#fa520f",
    deepseek: "#4d6bff", llama: "#0081fb", granite: "#2563eb", ernie: "#2932e1",
    glm: "#8b5cf6", nemotron: "#76b900", openai: "#10a37f", rnj: "#ec4899",
    olmo: "#e85d04", seed: "#f59e0b", lfm: "#6366f1", liquid: "#6366f1",
  };
  const getColor = (row) => {
    const f = (row.family || row.model_id || "").toLowerCase();
    for (const [key, col] of Object.entries(FAMILY_COLORS)) if (f.includes(key)) return col;
    return C.accent;
  };

  // Ordenar por pts efetivos (mais baixo primeiro para o tooltip sobrepor corretamente)
  const sorted = [...rows].sort((a, b) => (a.net_score || 0) - (b.net_score || 0));
  sorted.forEach((row) => {
    const x = toX(row.net_score || 0);
    const y = toY(row.accuracy ?? 0);
    const r = toR(row.scored || 0);
    const col = getColor(row);
  ...
  // Labels (apenas top 8 por pts efetivos + todos com accuracy > 0.9)
  const top8 = [...rows].sort((a, b) => (b.net_score || 0) - (a.net_score || 0)).slice(0, 8);
  const labeled = new Set(top8.map((r) => r.model_id));
  rows.filter((r) => (r.accuracy || 0) >= 0.9).forEach((r) => labeled.add(r.model_id));
  const labelRows = rows.filter((r) => labeled.has(r.model_id));

  labelRows.forEach((row) => {
    const x = toX(row.net_score || 0);
    const y = toY(row.accuracy ?? 0);
    const r = toR(row.scored || 0);
    const name = (row.display_name || row.model_id).replace(/^[^/]+\//, "");
    const tag  = `${row.net_score ?? "?"}pts, ${Math.round((row.accuracy || 0) * 100)}%`;
    const col  = getColor(row);

    ctx.fillStyle = C.text; ctx.font = `bold 10px system-ui`;
    ctx.textAlign = x > PAD.left + PW * 0.75 ? "right" : "left";
    const lx = x > PAD.left + PW * 0.75 ? x - r - 5 : x + r + 5;
    ctx.fillText(name, lx, y - 4);
    ctx.fillStyle = col; ctx.font = `9px system-ui`;
    ctx.fillText(tag, lx, y + 8);
  });
