...
def test_database_status(seeded_repo: DuckDBRepository) -> None:
    ...
    assert status["teams"] == 4
    assert status["matches"] == 20
    assert status["snapshot"] == "sample-2026-07-06"


def test_championship_table(seeded_repo: DuckDBRepository) -> None:
    rows = championship_table(seeded_repo, iterations=1000)
    assert len(rows) == 4
...
def test_team_analysis_view(seeded_repo: DuckDBRepository) -> None:
    view = team_analysis_view(seeded_repo, "T-NTL")
    assert view["team_id"] == "T-NTL"
...
def test_match_prediction_view(seeded_repo: DuckDBRepository) -> None:
    view = match_prediction_view(seeded_repo, "T-NTL", "T-SDR")
